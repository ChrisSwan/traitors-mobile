...
- **Empty `content` is a failure, not a response.** DeepSeek returns `content: ""` with `finish_reason: "length"` when the `max_tokens` budget is exhausted by reasoning before an answer is produced (verified live). `complete` must raise in that case — never return an empty string as if it were a real exchange, and never substitute `reasoning_content`.
...
- A mocked DeepSeek 2xx response with `content: ""` and non-empty `reasoning_content` → `complete` raises (a `BackendError` subclass), never returns `""`, never returns the reasoning text.
